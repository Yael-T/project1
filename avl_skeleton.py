...
class AVLTreeList(object):
    ...
    def split(self, i):
        x = self.treeSelect(self.getRoot(), i) # find the i'th node
        val = x.getValue()
        y = x.getParent()
        isLeftSon = True if x.isLeftSon() else False
        List1 = AVLTreeList()
        if x.getLeft().isRealNode():
            List1.setRoot(x.getLeft())
            x.getLeft().setParent(None)
            x.setLeft(AVLNode(None))
        List2 = AVLTreeList()
        if x.getRight().isRealNode():
            List2.setRoot(x.getRight())
            x.getRight().setParent(None)
            x.setRight(AVLNode(None))
            
        while y is not None:
            temp = AVLTreeList()
            if isLeftSon: #right turn going up
                if y.getRight().isRealNode():
                    temp.setRoot(y.getRight())
                x = y
                y = x.getParent()
                isLeftSon = True if x.isLeftSon() else False
                ...
                x.setLeft(AVLNode(None))
                x.setParent(None)
                #print("right: ", List2.listToArray(), x.getValue(), temp.listToArray())
                List2.joinToRight(x, temp)
                #print(List2.listToArray())
 
            else: #left turn going up
                if y.getLeft().isRealNode():
                    temp.setRoot(y.getLeft())
                x = y
                y = x.getParent()
                isLeftSon = True if x.isLeftSon() else False              
                ...
                x.setLeft(AVLNode(None))
                x.setParent(None)
                #print("left: ", temp.listToArray(), x.getValue(), List1.listToArray())
                List1.joinToLeft(x, temp)
                #print(List1.listToArray())
        #print(List1.listToArray(), val, List2.listToArray())
    ...
    def concat(self, lst):
        ...
        if self.empty():
            h = lst.getRoot().getHeight()
            self.setRoot(lst.getRoot())
            return h
        if self.getRoot().isLeaf():
            x = self.getRoot()
            self.delete(0)
            return self.joinToRight(x, lst)
        x = self.getLastNode()
        self.delete(self.length()-1)
        return self.joinToRight(x, lst)


    def joinToRight(self, x, other):
        ...
        if other.empty():
            ...
            self.fixTree(x.getParent())
            return h
        if self.empty():
            h = other.getRoot().getHeight()
        ...
        A = self.getRoot()
        B = other.getRoot()
        balanceFactor =  A.getHeight() - B.getHeight()
        if abs(balanceFactor) <= 1:
            ...
            while x.getParent() is not None:
                x.updateNodeInfo()
                x = x.getParent()
            self.fixTreeAfterDeletion(node)
        else: #balanceFactor < -1:
            while(B.getLeft().isRealNode() and B.getHeight() > A.getHeight()):
                B = B.getLeft()
            C = B.getParent()
            x.setLeft(A), A.setParent(x)
            x.setRight(B), B.setParent(x)
            ...
            self.setRoot(x)
            self.fixTreeAfterDeletion(node)
        return abs(other.getRoot().getHeight() - self.getRoot().getHeight())


    def joinToLeft(self, x, other):
        if other.empty() and self.empty():
            self.setRoot(x)
            x.updateNodeInfo()
            return 0
...
if __name__ == '__main__':
    n = 200
    for k in range(n): #first and last dont work
            smallTree = AVLTreeList()
            bigTree = AVLTreeList()
            for i in range(k):
                smallTree.insert(i, i)
            for i in range(k,n):
                bigTree.insert(i-k, i)
            smallTree.concat(bigTree)
            
            if smallTree.listToArray() != list(range(n)):
                print("concat: ", k, " error")
    ...
    for k in range(n):
        ...
        for i in range(n):
            tree.insert(i,i)
        splitted =tree.split(k)
        if splitted[0].listToArray()!= list(range(k)) or \
           splitted[1]!=k or splitted[2].listToArray()!= list(range(k+1,n)):
            print("split: ", k, " error")
